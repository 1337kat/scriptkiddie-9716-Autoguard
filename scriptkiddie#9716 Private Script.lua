--// Date 10/18/22
--// Key CTRL to Autoguard

local tr = false
local Mouse = game.Players.LocalPlayer:GetMouse()

game:GetService("UserInputService").InputBegan:Connect(function(key, gpe)
  if key.KeyCode == Enum.KeyCode.LeftControl and not gpe and tr == false then
          tr = true
          for i, v in pairs(game.Players:GetPlayers()) do
              ...
              if v.Name ~= char.Name and (v.Character.HumanoidRootPart.CFrame.p - char.HumanoidRootPart.CFrame.p).Magnitude < 25 then
                  while tr == true do
                      ...
                      elseif v.Character.HumanoidRootPart.Velocity.magnitude < 0.5 and v.Character:FindFirstChild("Basketball") then
      ...
      elseif key.KeyCode == Enum.KeyCode.LeftControl and not gpe and tr == true then
          tr = false
      end
  end
)
...
local window = lib:MakeWindow({Name = "Lifetime Premium Cracked by scriptkiddie#9716"})
local main = window:MakeTab({Name = ""})
